...
const CATEGORY_KEYWORDS = {
  'Food & Grocery': ['food', 'lunch', 'dinner', 'breakfast', 'coffee', 'grocery', 'groceries', 'meal', 'snack', 'drink', 'milktea', 'milk tea', 'grab food', 'foodpanda', 'jollibee', 'mcdo', 'kfc', 'pizza', 'cake', 'bread', 'rice', 'water bottle', 'juice', 'soda'],
  'Utilities': ['electricity', 'electric', 'water', 'internet', 'wifi', 'gas', 'membership', 'subscription', 'netflix', 'spotify', 'globe', 'smart', 'pldt', 'converge'],
  'CC': ['credit card', 'card', 'shopee', 'lazada', 'gcash', 'maya', 'paymaya'],
};
...
export default {
  data: new SlashCommandBuilder()
    .setName('expense')
    .setDescription('Log an expense to your budget spreadsheet')
    .addStringOption((option) =>
      option.setName('description').setDescription('What was the expense for?').setRequired(true)
    )
    .addNumberOption((option) =>
      option.setName('amount').setDescription('Amount spent (e.g. 180)').setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName('date')
  ...
  execute: withErrorHandling(
    async (interaction, config, client) => {
      ...
      try {
        ...
        logger.info(
          `Expense logged via slash command: ${interaction.user.id} / ${description} / ${amount} / HTTP sent`
        );
        ...
        logger.error(`Failed to send expense to workflow${status ? ` (${status})` : ''}:`, detail);
